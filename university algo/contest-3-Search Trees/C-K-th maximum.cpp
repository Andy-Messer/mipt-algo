...
public:
    ...
    virtual long long K_th(NodeTree *tempRoot, long long &a) {
        long long delta = nodeRoot->GetSize() - a + 1;
        long long left = 1, right = nodeRoot->GetSize();
        long long numNodes;
        for (;;) {
            numNodes = left + tempRoot->leftsize;
            if (numNodes < delta) {
                left += tempRoot->leftsize + 1;
                tempRoot = tempRoot->GetRightChild();
            } else {
                if (numNodes > delta) {
                    right += tempRoot->rightsize - 1;
...
public:
    ...
    virtual long long K_th(long long &a);//К-ый максимум
...
long long TreeAVL::K_th(long long &a) {
    return _treeAVL.K_th( _treeAVL.nodeRoot,a);
}
